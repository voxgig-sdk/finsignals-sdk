...
// Finsignals SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class FinsignalsMakeError
{
    public static function call(?FinsignalsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FinsignalsContext([], null);
        }
        $op = $ctx->op ?? new FinsignalsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FinsignalsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FinsignalsError) ? $err->msg : (string)$err;
        $msg = "FinsignalsSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new FinsignalsError('', $msg, $ctx);
        ...
        if ($err instanceof FinsignalsError) {
