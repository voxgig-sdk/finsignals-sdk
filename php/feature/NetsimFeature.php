...
class FinsignalsNetsimFeature extends FinsignalsBaseFeature
{
    ...
    private function _simulate(FinsignalsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FinsignalsContext $ctx, array $applied): void
    {
        $client = $this->client;
