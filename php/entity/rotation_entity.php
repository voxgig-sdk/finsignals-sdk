...
// Finsignals SDK Rotation entity
...
class RotationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RotationEntity($this->_client, $opts);
    }

    /**
     * @param Rotation|array $args Rotation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = FinsignalsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Rotation|array The current Rotation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Rotation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = FinsignalsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Rotation fields).
     */
    ...
    /**
     * Load a single Rotation.
     *
     * @param RotationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RotationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Rotation|array The loaded Rotation as an assoc-array at the
     *   SDK boundary; throws FinsignalsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FinsignalsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
