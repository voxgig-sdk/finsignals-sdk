...
{
    private static function utility(): FinsignalsUtility
    {
        return new FinsignalsUtility();
    }
    ...
    private static function ctx(array $over = []): FinsignalsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FinsignalsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FinsignalsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FinsignalsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FinsignalsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FinsignalsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FinsignalsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FinsignalsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FinsignalsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FinsignalsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinsignalsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FinsignalsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FinsignalsResponse([]),
            'result' => new FinsignalsResult([]),
        ]);
        [, $err] = FinsignalsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FinsignalsSpec([]),
            'response' => null,
            'result' => new FinsignalsResult([]),
        ]);
        [, $err] = FinsignalsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FinsignalsSpec(['step' => 's']),
            'response' => new FinsignalsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FinsignalsResult([]),
        ]);
        [, $err] = FinsignalsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FinsignalsSpec(['step' => 's']),
            'response' => new FinsignalsResponse(self::resp(200, ['v' => 1])),
            'result' => new FinsignalsResult([]),
        ]);
        [, $err] = FinsignalsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FinsignalsSpec(['step' => 's']),
            'response' => new FinsignalsResponse(self::resp(200, ['v' => 2])),
            'result' => new FinsignalsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FinsignalsMakeResponse::call($ctx);
    ...
    {
        $preset = new FinsignalsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FinsignalsSpec([]),
            'response' => new FinsignalsResponse([]),
            'result' => new FinsignalsResult([]),
        ...
        [$got, $err] = FinsignalsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FinsignalsResult([])]);
        [, $err] = FinsignalsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FinsignalsSpec([]), 'result' => null]);
        [, $err] = FinsignalsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinsignalsSpec(['step' => 's']),
            'result' => new FinsignalsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FinsignalsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinsignalsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FinsignalsSpec(['step' => 's']),
            'result' => new FinsignalsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FinsignalsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FinsignalsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FinsignalsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FinsignalsSpec([]),
            'result' => new FinsignalsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FinsignalsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinsignalsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FinsignalsError('boom', 'boom');
        $utility->fetcher = function (FinsignalsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinsignalsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FinsignalsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FinsignalsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FinsignalsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FinsignalsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        FinsignalsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FinsignalsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FinsignalsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FinsignalsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FinsignalsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FinsignalsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FinsignalsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FinsignalsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FinsignalsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FinsignalsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FinsignalsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FinsignalsResult(['ok' => false])]);
        $this->expectException(FinsignalsError::class);
        FinsignalsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FinsignalsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FinsignalsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FinsignalsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FinsignalsBaseFeature();
        $b = new FinsignalsBaseFeature();
        FinsignalsFeatureAdd::call($ctx, $a);
        FinsignalsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FinsignalsBaseFeature
    {
        $f = new FinsignalsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FinsignalsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FinsignalsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FinsignalsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FinsignalsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        FinsignalsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FinsignalsResponse(['headers' => null]),
            'result' => new FinsignalsResult([]),
        ]);
        FinsignalsResultHeaders::call($ctx);
