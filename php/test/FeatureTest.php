...
class FtCtrl extends FinsignalsControl
{
...
class FtClient
{
    ...
    private FinsignalsUtility $utility;

    public function __construct(FinsignalsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FinsignalsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FinsignalsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FinsignalsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FinsignalsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FinsignalsConfig::make_feature($name);
    ...
    private static function buildUrl(FinsignalsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FinsignalsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FinsignalsResponse($fetched);
        }
    ...
    private function fail(FinsignalsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FinsignalsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
