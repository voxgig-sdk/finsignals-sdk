...
class FinsignalsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FinsignalsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FinsignalsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FinsignalsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FinsignalsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FinsignalsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FinsignalsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FinsignalsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FinsignalsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FinsignalsError($op . "_allow",
                "FinsignalsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FinsignalsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FinsignalsError("graphql_error",
                "FinsignalsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Classify()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->classify()
    // resolves here too.
    public function Classify($data = null)
    {
        require_once __DIR__ . '/entity/classify_entity.php';
        if ($data === null) {
            if ($this->_classify === null) {
                $this->_classify = new ClassifyEntity($this, null);
            }
            return $this->_classify;
        }
        return new ClassifyEntity($this, $data);
    }
    ...
    // Canonical facade: $client->Internal()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->internal()
    // resolves here too.
    public function Internal($data = null)
    {
        require_once __DIR__ . '/entity/internal_entity.php';
        if ($data === null) {
            if ($this->_internal === null) {
                $this->_internal = new InternalEntity($this, null);
            }
            return $this->_internal;
        }
        return new InternalEntity($this, $data);
    }


    private $_rotation = null;

    // Canonical facade: $client->Rotation()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->rotation()
    // resolves here too.
    public function Rotation($data = null)
    {
        require_once __DIR__ . '/entity/rotation_entity.php';
        if ($data === null) {
            if ($this->_rotation === null) {
                $this->_rotation = new RotationEntity($this, null);
            }
            return $this->_rotation;
        }
        return new RotationEntity($this, $data);
    }
