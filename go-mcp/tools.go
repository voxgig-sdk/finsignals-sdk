...
type Args struct {
	Entity string         `json:"entity" jsonschema:"classify | health | internal | rotation | usage"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.FinsignalsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "finsignals_list",
		Description: "List records from Finsignals. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "finsignals_load",
		Description: "Load a single record from Finsignals. " +
...
func runOp(client *sdk.FinsignalsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.FinsignalsSDK, name string) (sdk.FinsignalsEntity, error) {
	switch strings.ToLower(name) {
	case "classify":
		return client.Classify(nil), nil
	case "health":
		return client.Health(nil), nil
	case "internal":
		return client.Internal(nil), nil
	case "rotation":
		return client.Rotation(nil), nil
	case "usage":
