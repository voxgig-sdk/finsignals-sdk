...
[MCP](https://modelcontextprotocol.io) server exposing the Finsignals SDK as
two agent tools — `finsignals_list` and `finsignals_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/finsignals-mcp)
make build

# 2. Provide credentials via the environment
export FINSIGNALS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user finsignals \
  -- /absolute/path/to/finsignals-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./finsignals-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// finsignals_list: first page of records
{ "entity": "classify" }
{ "entity": "classify", "query": { } }

// finsignals_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/finsignals-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user finsignals \
     -- "$PWD"/dist/*/finsignals-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `finsignals_list` and `finsignals_load` tools now appear
   in new sessions. Ask the agent to *"list classify using finsignals"*
   and it calls `finsignals_list` with `{"entity":"classify"}`.
...
```sh
export FINSIGNALS_APIKEY=sk_live_xxx            # API key
export FINSIGNALS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `finsignals_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `finsignals_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FINSIGNALS_APIKEY` | API key sent with every request. |
| `FINSIGNALS_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

classify | health | internal | rotation | usage

### Smoke test via HTTP (raw JSON-RPC)

```sh
./finsignals-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"finsignals_load","arguments":{"entity":"health","query":{"id":1}}}}'
```
