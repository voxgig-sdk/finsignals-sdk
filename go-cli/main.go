// Command Finsignals-cli is a CLI + REPL for the Finsignals SDK,
...
const prompt = "finsignals"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "classify health internal rotation usage"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FINSIGNALS_APIKEY (api key), FINSIGNALS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FINSIGNALS_APIKEY carries the API key and
	// FINSIGNALS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FINSIGNALS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FINSIGNALS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFinsignalsSDK(opts)
