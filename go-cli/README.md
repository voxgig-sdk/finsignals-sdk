...
boru-driven command-line client **and** interactive REPL for the Finsignals
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/finsignals-cli)
make build
...
FINSIGNALS_BASE=https://api.example.com ./finsignals-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/finsignals-cli
   ```
...
Run with no arguments to open a REPL (prompt `finsignals>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `classify`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/finsignals-cli`. |
...
(`./finsignals-cli <expr>`) and interactively (the REPL), and expressions compose the
