...
module FinsignalsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FinsignalsContext.new({}, nil)
    end
    op = ctx.op || FinsignalsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FinsignalsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FinsignalsError) ? err.msg : err.to_s
    ...
    sdk_err = FinsignalsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FinsignalsError)
