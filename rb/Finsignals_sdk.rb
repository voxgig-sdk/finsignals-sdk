...
require_relative 'Finsignals_types'
...
class FinsignalsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FinsignalsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FinsignalsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FinsignalsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FinsignalsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FinsignalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FinsignalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FinsignalsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FinsignalsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FinsignalsError.new(
        "graphql_error", "FinsignalsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Classify.list / client.Classify.load({ "id" => ... })
  def Classify(data = nil)
    require_relative 'entity/classify_entity'
    ClassifyEntity.new(self, data)
  end
  ...
  # Canonical facade: client.Internal.list / client.Internal.load({ "id" => ... })
  def Internal(data = nil)
    require_relative 'entity/internal_entity'
    InternalEntity.new(self, data)
  end


  # Canonical facade: client.Rotation.list / client.Rotation.load({ "id" => ... })
  def Rotation(data = nil)
    require_relative 'entity/rotation_entity'
    RotationEntity.new(self, data)
  end
