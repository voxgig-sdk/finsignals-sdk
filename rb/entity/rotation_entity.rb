...
class RotationEntity
  ...
  def make
    opts = @_entopts.dup
    RotationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FinsignalsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Rotation, Hash] the current Rotation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FinsignalsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Rotation fields)
  def match_get
  ...
  #
  # @param reqmatch [RotationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Rotation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Rotation, Hash] the loaded Rotation; raises FinsignalsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FinsignalsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
