...
module FinsignalsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FinsignalsResponse.new({})
    ...
    if fetch_err
      ...
      response = FinsignalsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FinsignalsResponse.new(fetched)
    else
