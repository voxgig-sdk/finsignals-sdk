...
import (
	...
	omni "github.com/voxgig-sdk/finsignals-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FinsignalsError); is {
				out := map[string]any{
					"name":    "FinsignalsError",
...
func omniCtx(arg any, client *sdk.FinsignalsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FinsignalsSDK); is {
			client = live
