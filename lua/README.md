...
The Lua SDK for the Finsignals API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Classify()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/finsignals-sdk/releases)),
...
Create an instance: `local classify = client:Classify(nil)`
...
```lua
local classify, err = client:Classify():create({
...
Create an instance: `local internal = client:Internal(nil)`
...
```lua
local internal, err = client:Internal():create({
})
```
...
Create an instance: `local rotation = client:Rotation(nil)`
...
| `generated_at` | `string` |  |
| `model_version` | `string` |  |
| `outlook_1y` | `table` | Data for one analysis period (1y or 5y). |
| `outlook_5y` | `table` | Data for one analysis period (1y or 5y). |
| `request_id` | `string` |  |
| `trading_date` | `string` |  |
...
```lua
local rotation, err = client:Rotation():load()
```
...
The main module (`finsignals_sdk`) exports the SDK constructor
