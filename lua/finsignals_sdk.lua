...
local FinsignalsSDK = {}
FinsignalsSDK.__index = FinsignalsSDK
...
FinsignalsSDK._make_feature = _make_feature


function FinsignalsSDK.new(options)
  local self = setmetatable({}, FinsignalsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function FinsignalsSDK:options_map()
...
function FinsignalsSDK:get_utility()
  return Utility.copy(self._utility)
end


function FinsignalsSDK:get_root_ctx()
  return self._rootctx
end


function FinsignalsSDK:prepare(fetchargs)
...
function FinsignalsSDK:direct(fetchargs)
...
-- by passing it.
function FinsignalsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function FinsignalsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "FinsignalsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:Classify():list() / client:Classify():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FinsignalsSDK:Classify(data)
  local EntityMod = require("entity.classify_entity")
  if data == nil then
    if self._classify == nil then
      self._classify = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FinsignalsSDK:Health(data)
...
-- Idiomatic facade: client:Internal():list() / client:Internal():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FinsignalsSDK:Internal(data)
  local EntityMod = require("entity.internal_entity")
  if data == nil then
    if self._internal == nil then
      self._internal = EntityMod.new(self, nil)
    end
...
-- Idiomatic facade: client:Rotation():list() / client:Rotation():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FinsignalsSDK:Rotation(data)
  local EntityMod = require("entity.rotation_entity")
  if data == nil then
    if self._rotation == nil then
      self._rotation = EntityMod.new(self, nil)
    end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function FinsignalsSDK:Usage(data)
...
function FinsignalsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = FinsignalsSDK.new(sdkopts)
  sdk.mode = "test"
