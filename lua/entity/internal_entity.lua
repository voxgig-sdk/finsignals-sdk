...
local InternalEntity = {}
InternalEntity.__index = InternalEntity


function InternalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InternalEntity)
  self._name = "internal"
  self._client = client
...
function InternalEntity:get_name()
  return self._name
end


function InternalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InternalEntity.new(self._client, opts)
end
...
function InternalEntity:mark_deleted()
  self._deleted = true
end


function InternalEntity:deleted()
  return true == self._deleted
end


function InternalEntity:data_set(args)
  if args ~= nil then
...
function InternalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InternalEntity:match_set(args)
  if args ~= nil then
...
function InternalEntity:match_get()
...
function InternalEntity:stream(action, args, callopts)
...
---@param reqdata InternalCreateData
---@param ctrl? table
---@return Internal
---@return string? err
function InternalEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function InternalEntity:_run_op(ctx, post_done)
  local utility = self._utility
