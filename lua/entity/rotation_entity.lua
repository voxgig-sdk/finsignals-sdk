...
local RotationEntity = {}
RotationEntity.__index = RotationEntity


function RotationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RotationEntity)
  self._name = "rotation"
  self._client = client
...
function RotationEntity:get_name()
  return self._name
end


function RotationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RotationEntity.new(self._client, opts)
end
...
function RotationEntity:mark_deleted()
  self._deleted = true
end


function RotationEntity:deleted()
  return true == self._deleted
end


function RotationEntity:data_set(args)
  if args ~= nil then
...
function RotationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RotationEntity:match_set(args)
  if args ~= nil then
...
function RotationEntity:match_get()
...
function RotationEntity:stream(action, args, callopts)
...
---@param reqmatch RotationLoadMatch
---@param ctrl? table
---@return Rotation
---@return string? err
function RotationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function RotationEntity:_run_op(ctx, post_done)
  local utility = self._utility
