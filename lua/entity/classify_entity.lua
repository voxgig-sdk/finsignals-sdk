...
local ClassifyEntity = {}
ClassifyEntity.__index = ClassifyEntity


function ClassifyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClassifyEntity)
  self._name = "classify"
  self._client = client
...
function ClassifyEntity:get_name()
  return self._name
end


function ClassifyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClassifyEntity.new(self._client, opts)
end
...
function ClassifyEntity:mark_deleted()
  self._deleted = true
end


function ClassifyEntity:deleted()
  return true == self._deleted
end


function ClassifyEntity:data_set(args)
  if args ~= nil then
...
function ClassifyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClassifyEntity:match_set(args)
  if args ~= nil then
...
function ClassifyEntity:match_get()
...
function ClassifyEntity:stream(action, args, callopts)
...
---@param reqdata ClassifyCreateData
---@param ctrl? table
---@return Classify
---@return string? err
function ClassifyEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function ClassifyEntity:_run_op(ctx, post_done)
  local utility = self._utility
