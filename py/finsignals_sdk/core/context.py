...
from finsignals_sdk.core.control import FinsignalsControl
...
from finsignals_sdk.core.helpers import get_ctx_prop, to_map
...
class FinsignalsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, FinsignalsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, FinsignalsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, FinsignalsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return FinsignalsError(code, msg, self)
