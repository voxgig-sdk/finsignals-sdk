...
from finsignals_sdk.utility import register

# Load features
from finsignals_sdk.feature.base_feature import FinsignalsBaseFeature
from finsignals_sdk.features import _has_feature, _make_feature
...
class FinsignalsSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = FinsignalsUtility()
        self._utility = utility

        from finsignals_sdk.config import shared_config
    ...
    def get_utility(self):
        return FinsignalsUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("FinsignalsSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Classify(self, data=None) -> "ClassifyEntity":
        """Entity factory: client.Classify().list() / client.Classify().load({"id": ...})."""
        from finsignals_sdk.entity.classify_entity import ClassifyEntity
        return ClassifyEntity(self, data)


    def Health(self, data=None) -> "HealthEntity":
        """Entity factory: client.Health().list() / client.Health().load({"id": ...})."""
        from finsignals_sdk.entity.health_entity import HealthEntity
        return HealthEntity(self, data)


    def Internal(self, data=None) -> "InternalEntity":
        """Entity factory: client.Internal().list() / client.Internal().load({"id": ...})."""
        from finsignals_sdk.entity.internal_entity import InternalEntity
        return InternalEntity(self, data)


    def Rotation(self, data=None) -> "RotationEntity":
        """Entity factory: client.Rotation().list() / client.Rotation().load({"id": ...})."""
        from finsignals_sdk.entity.rotation_entity import RotationEntity
        return RotationEntity(self, data)


    def Usage(self, data=None) -> "UsageEntity":
        """Entity factory: client.Usage().list() / client.Usage().load({"id": ...})."""
        from finsignals_sdk.entity.usage_entity import UsageEntity
        return UsageEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "FinsignalsSDK":
        if sdkopts is None:
            sdkopts = {}
