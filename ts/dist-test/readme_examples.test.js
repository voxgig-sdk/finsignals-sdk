...
const TEST_SEED = { "entity": { "classify": { "test01": { "id": "test01" } }, "health": { "test01": { "id": "test01" } }, "internal": { "test01": { "id": "test01" } }, "rotation": { "test01": { "id": "test01" } }, "usage": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+FinsignalsSDK\b/.test(code) ||
        /\bFinsignalsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FinsignalsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FinsignalsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
