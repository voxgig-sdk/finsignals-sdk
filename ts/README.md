# Finsignals TypeScript SDK



The TypeScript SDK for the Finsignals API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Classify()` — each with a small set of operations (`load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `FinsignalsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FinsignalsSDK.test(testopts?, sdkopts?)` | `FinsignalsSDK` | Create a test-mode client. |
...
| `client` | `client(): FinsignalsSDK` | Return the parent SDK client. |
...
Create an instance: `const classify = client.Classify()`
...
Create an instance: `const internal = client.Internal()`
...
```ts
const internal = await client.Internal().create({
})
```
...
Create an instance: `const rotation = client.Rotation()`
...
```ts
const rotation = await client.Rotation().load()
```
...
finsignals/
├── src/
│   ├── FinsignalsSDK.ts        # Main SDK class
